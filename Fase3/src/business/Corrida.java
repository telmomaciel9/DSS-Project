...
public class Corrida implements Serializable
{
   ...
   /**
    ...
    public void simulacao()
    {
        ...
        for(int i=0;i<voltas;i++){
            for (int j=0;j<this.listaCarros.size()-1;j++){
                ...
                Carro c2 = this.listaCarros.get(j+1);

                for(int gdu : gdus){
                  Boolean  ultra = ultrapassagem(c1,c2,gdu);
                  if (ultra == true){
                    this.listaCarros.set(j,c2);
                    this.listaCarros.set(j+1,c1);
                  }
                }
    ...
    public Boolean ultrapassagem(Carro c1, Carro c2, int gdu){
        if (gdu == 3){
            return false;
        }
        if (gdu == 2 && c2.getPotencia() > c1.getPotencia() && c2.getCilindrada() > c1.getCilindrada()){
            return true;
        }
        if (gdu == 1 && c2.getCilindrada() > c1.getCilindrada()){
            return true;
        }
        if (gdu == 2 && c2.getPotencia() > c1.getPotencia() && c2.getModoMotor() == "Agressivo" && c1.getModoMotor() != "Agressivo"){
            return true;
        } 
        if (gdu == 1 && c2.getPotencia() > c1.getPotencia()){
            return true;
        }
     ...
     * Simula a corrida
     */   
    public void simulaCorridaPremium()
    {
        int voltas = this.circuito.getVoltas();
        long t_aux, t_volta;
        ArrayList<Carro> aux = new ArrayList<Carro>();
    ...
    /**
     * Metodo auxiliar privado para determinar o carro que vai em 1o a ctempoBoxda volta
     */
